...
!!                                  |depressional areas (not used)
!!    imp_trig(:,:,:)|none          |release/impound action code:
!!                                  |0 begin impounding water
!!                                  |1 release impounded water
!!    irelease(:,:,:)|julian date   |date of impound/release operation (not used)
!!    laiday(:)      |none          |leaf area index
!!    nrelease(:)    |none          |sequence number of impound/release
!!                                  |operation within the year (not used)
!!    nro(:)         |none          |sequence number of year in rotation (not used)
...
!!                                  |impoundment after settling for one day (not used)
...
!!                                  |a fraction of the total volume (not used)
...
!       track incoming loads, seems useless. LJ
...
!        latno3(j) = latno3(j) * yy  ! commented by LJ
!        gwno3(j) = gwno3(j) * yy
